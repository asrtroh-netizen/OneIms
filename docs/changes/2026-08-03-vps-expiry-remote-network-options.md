...
**降配后续费或换同价轻量**，NPS+反代原样保留。¥300 若已是轻量档，续费往往比「搬家+调 DNS+验三地」更省时间。

### B. 想明显更便宜 / 少养服务器

1. **Tailscale** 覆盖三地飞牛 + 办公室 PC（SSH/跳板/互访）→ 年费可 **¥0**  
2. 公网站点（`sub.itt.fan`、对外面板）：**Cloudflare Tunnel** 挂在有服务的飞牛上，或买 **更小的海外/竞品 VPS** 只跑反代  
3. 阿里云到期可释放，DNS 按新入口改

### C. 折中

续费但 **立刻降配** 到 NPS+nginx 所需最低规格；同时试点 Tailscale，确认稳定后再考虑退掉部分公网映射。 
