...
1. **先共存**：现网阿里云 NPS+反代 **保持不动**；并行上 Tailscale。  
2. **验稳再撤**：SSH + 飞牛网页在 Tailscale 上用顺后，再停 NPS / 不续阿里云。  
3. **时间窗**：阿里云大约 **十月到期**，足够双轨验证。  
4. **备用机**：女朋友侧可购 **首年约 ¥68** 的机器作保险（公网反代/NPS 迁徙备用），**不必立刻切主**。

### 共存期验收（通过再考虑撤 NPS）

- [ ] 三地飞牛 + 办公 PC 均在 Tailscale 在线  
- [ ] 办公 PC → 各飞牛 SSH 稳定  
- [ ] 浏览器经 `100.x`（或私有域名）打开飞牛管理页可点选操作  
- [ ] 连续使用 ≥1～2 周无「必须靠 NPS 才能办的事」（除明确要保留的公网站）

### 建议落地顺序

1. 先装 **TTFN** 试点 → 办公室 PC 接入 → 验 SSH/网页  
2. 再装 **FNHOME / HaloXFN**、办公室飞牛 DDOS  
3. （可选）整理 `itt.fan` 私有解析  
4. 十月前决定：只留 Tailscale，或把 NPS 迁到 ¥68 备用机
