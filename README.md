...
**本版只发 OneIms Lite。** 3.4.0 之后的抓 Bug 三轮、包体裁剪和产品线收口都打进这个包；OneKuku 不再出新包。

* 🛡️ **写入更老实**：一键恢复清不掉时改报失败，不再假成功；「部分失败」的配置不再每次开机重放；清空没生效会提示 mismatch。
* 📱 **双卡体检**：副卡 / 非数据卡按本卡判断，SIM 或语音变差会回滚；非数据卡不再被数据项拖成「不健康」。
* ⬇️ **应用内更新**：后台下载完会弹出「下载完成，点击安装」；GitHub Release 缺本渠道 APK 时不再误下另一条线的包。
* 📝 成功提示改为「优先 persistent」；能力页授权过期时明确说「需要授权」。
* 📦 正式包更小（约 2.9 MB；3.4.0 Lite 约 3.6 MB）：只留中英 locale、正式包仅 arm64。
* 🔒 **OneKuku 封存**：内嵌通道线停更，请改用 Lite + Shizuku。
...
* 🔵 OneIms Lite：覆盖安装 `OneIms-Lite-Shizuku-3.5.0.apk`
* ⚪ OneKuku：无本版。请继续用 3.4.0，或卸载后改装 Lite
* versionCode `115`（相对 3.4.0 的 `114` 会提示升级）

---

## ✨ What's New · 3.4.0

* 🎯 **面向 Android 17**：`targetSdk` 升到 37（与 `compileSdk` 对齐）。开机前台服务仍走 `specialUse`；16KB 页对齐已按 Pixel 新机要求打好。
* 🟢 **OneKuku**：无 shell-uid 时不再用 App UID 空跑 `cmd phone`；开 NR 第三级改走类型化 phone 命令（旧 Bridge 会软失败提示，而不是假装成功）。
* 🔵 **OneIms Lite**：同版号同步；Sui 授权框冷却仍沿用 3.3.12 的修复。
...
> ⚠️ **从 3.3.4 及更早版本升级**：那些版本是调试签名，Android 不允许跨签名覆盖，请**先卸载旧包再安装**；App 内的「下载并安装」在最后一步会失败，这是签名限制不是包坏了。卸载会清空本地设置，重装后需重新应用方案。
...
> ⚠️ **本版换了签名，不能覆盖安装**：Android 拒绝跨签名升级。**包括早前下载过 3.3.5 的用户在内，所有老用户都要先卸载旧包、再安装本页新包**；应用内更新对旧包同样会失败。卸载会清空本地设置，重装后需重新应用方案。
